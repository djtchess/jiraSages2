...
export class EpicDurationChartComponent implements OnInit, OnChanges {
  ...
  displayedColumns: string[] = ['epicKey', 'epicSummary', 'status', 'versions', 'sprints', 'storyPoints', 'days', 'velocity', 'developers', 'tickets'];
...
function render(){const rows=getFiltered();document.getElementById('kpiEpics').textContent=String(rows.length);document.getElementById('kpiVersions').textContent=String(allVersions.length);document.getElementById('kpiSprints').textContent=String(allSprints.length);const tbody=document.getElementById('rows');tbody.innerHTML='';rows.forEach((r,idx)=>{const tr=document.createElement('tr');tr.innerHTML='<td><a href="#" data-idx="'+idx+'">'+r.epicKey+'</a></td><td>'+r.epicSummary+'</td><td>'+r.status+'</td><td>'+(r.versionNames||[]).join(', ')+'</td><td>'+(r.sprintDeliveries||[]).map(s=>s.sprintName).join(' / ')+'</td><td>'+Number(r.totalStoryPoints||0).toFixed(2)+'</td><td>'+Number(r.totalTimeSpentDays||0).toFixed(2)+'</td><td>'+Number(r.epicVelocity||0).toFixed(2)+'</td><td>'+(r.developers||[]).join(', ')+'</td>';tbody.appendChild(tr);});tbody.querySelectorAll('a[data-idx]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();selectedEpicIndex=Number(a.getAttribute('data-idx'));renderTicket(rows);}));chart.setOption({tooltip:{trigger:'axis',axisPointer:{type:'shadow'}},legend:{data:['Nb sprints','Nb versions']},xAxis:{type:'category',data:rows.map(r=>r.epicKey),axisLabel:{rotate:35}},yAxis:{type:'value'},series:[{name:'Nb sprints',type:'bar',data:rows.map(r=>(r.sprintDeliveries||[]).length)},{name:'Nb versions',type:'bar',data:rows.map(r=>(r.versionNames||[]).length)}]});renderTicket(rows);}versionFilter.addEventListener('change',()=>{selectedVersions=Array.from(versionFilter.selectedOptions).map(o=>o.value);selectedEpicIndex=0;render();});sprintFilter.addEventListener('change',()=>{selectedSprints=Array.from(sprintFilter.selectedOptions).map(o=>o.value);selectedEpicIndex=0;render();});document.getElementById('resetBtn').addEventListener('click',()=>{selectedVersions=[];selectedSprints=[];Array.from(versionFilter.options).forEach(o=>o.selected=false);Array.from(sprintFilter.options).forEach(o=>o.selected=false);selectedEpicIndex=0;render();});render();</script></body></html>`;
  ...
  formatVelocity(row: EpicDeliveryOverview): string {
    return Number(row.epicVelocity || 0).toFixed(2);
  }
